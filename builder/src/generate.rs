...
use super::{
    ...
    BuildOutput, ConversionType, Overlay, Tree,
};
...
impl Generater {
    ...
    pub fn generate(
        ...
        let node_info = NodeInfo {
            ...
            src_dev: 0,
            src_ino: 0,  // 你需要提供一个适当的 Inode 值
            rdev: 0,
            source: PathBuf::from("/"),
            path: PathBuf::from("/"),
            target: PathBuf::from("/"),
            target_vec: vec![OsString::from("target"), OsString::from("rafs"), OsString::from("filesystem")],
            symlink: None,
        ...
        // chunk
        for _chunk in ctx.chunkdict.iter() {
            let chunk_wrapper = ChunkWrapper::new(RafsVersion::V6);
            // chunk_wrapper.set_index(chunk.clone());
            let chunk_wrapper_arc = Arc::new(chunk_wrapper);
            let node_chunk = NodeChunk {
                source: ChunkSource::Build,  // 或 ChunkSource::Dict, ChunkSource::Parent
                inner: chunk_wrapper_arc,
            };
            node.chunks.push(node_chunk);
        }
        let chunk_wrapper = ChunkWrapper::new(RafsVersion::V6);
        ...
        let blob_mgr = BlobManager::new(ctx.digester);
        let fs_version = RafsVersion::V6;
    ...
    pub fn generate_back(
        ...
        // node.set_inode();
        // node.set_parent();
        // node.set_child_index();
        // node.set_child_count();
        // node.set_dir_offset();
        // node.set_hardlink_offset();
        //
        // 2. 每个Layer的bootstrap，都是一个Tree，Tree的逻辑结构
        // let mut tree = Tree::new(node);
        // tree.insert_child(tree2);
        //
        // 3. Tree的落盘
        // 需要构建一个BootstrapContext，用于落盘
        if sources.is_empty() {
        ...
        for (layer_idx, bootstrap_path) in sources.iter().enumerate() {
            ...
            let chunkdict = &ctx.chunkdict;
            let chunk_dict_set: HashSet<String> = chunkdict.iter().cloned().collect();

            let upper = Tree::from_bootstrap(&rs, &mut ())?;
            upper.walk_bfs(true, &mut |n| {
                let mut node = n.lock_node();
                let filtered_chunks: Vec<_> = node
                    .chunks
                    .iter()
                    .filter(|&chunk| chunk_dict_set.contains(&chunk.inner.id().to_string()))
                    .cloned()
                    .collect();

                node.chunks = filtered_chunks;
