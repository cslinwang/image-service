...
impl SqliteDatabase {
    pub fn new(database_url: &str) -> Result<Self, rusqlite::Error> {
        ...
        let chunk_table_result = ChunkTable::new(database_url);
        let chunk_table = match chunk_table_result {
            Ok(table) => table,
            Err(e) => {
        ...
        let blob_table_result = BlobTable::new(database_url);
        let blob_table = match blob_table_result {
            Ok(table) => table,
            Err(e) => {
        ...
        Ok(Self {
            chunk_table: Some(chunk_table),
            blob_table: Some(blob_table),
        })
    }

    pub fn new_in_memory() -> Result<Self, rusqlite::Error> {
        let chunk_table_result = ChunkTable::new_in_memory();
        let chunk_table = match chunk_table_result {
            Ok(table) => table,
            Err(e) => {
                println!("An error occurred when creating the ChunkTable: {}", e);
...
impl Deduplicate<SqliteDatabase> {
    pub fn new(bootstrap_path: &Path, config: Arc<ConfigV2>, db_url: &str) -> anyhow::Result<Self> {
        ...
        let db = if db_url == "/sqlite_memory_model" {
...
mod tests {
    ...
    fn test_blob_table() -> Result<(), Box<dyn std::error::Error>> {
        ...
        {
            let conn = blob_table.conn.lock().map_err(|_| "Mutex poisoned")?;
            conn.execute(
                "CREATE TABLE IF NOT EXISTS blob (
                    id                      INTEGER PRIMARY KEY,
                    blob_id                 TEXT NOT NULL,
    ...
    fn test_chunk_table() -> Result<(), Box<dyn std::error::Error>> {
        ...
        {
            let conn = chunk_table.conn.lock().map_err(|_| "Mutex poisoned")?;
            conn.execute(
                "CREATE TABLE IF NOT EXISTS chunk (
                    id                      INTEGER PRIMARY KEY,
                    chunk_blob_id           TEXT NOT NULL,
