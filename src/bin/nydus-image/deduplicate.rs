...
impl Algorithm<SqliteDatabase> {
    ...
    fn exponential_smoothing(all_chunks: Vec<ChunkdictChunkInfo>, threshold: f64) -> anyhow::Result<Vec<ChunkdictChunkInfo>> {
        let alpha = 0.5;
    ...
    fn distance(image1: &[ChunkdictChunkInfo], image2: &[ChunkdictChunkInfo]) -> anyhow::Result<f64> {
        // The total size of all chunks in both images
        let mut image1_size = 0;
        let mut image2_size = 0;

        for chunk1 in image1 {
            image1_size += chunk1.chunk_compressed_size;
        }
        for chunk2 in image2 {
            image2_size += chunk2.chunk_compressed_size;
        }
        ...
        for chunk in all_chunks {
            if processed_digests.contains(&chunk.chunk_digest) {
                let size = compressed_size_map
                    ...
                    .or_insert(0);
                *size += chunk.chunk_compressed_size;
            }
            processed_digests.insert(&chunk.chunk_digest);
        }

        let repeat_size: u32 = compressed_size_map.values().cloned().sum();
    ...
    fn devide_set(
        ...
        for (_, chunk_list) in image_chunks.iter_mut() {
            let mut version_chunks: BTreeMap<CustomString, Vec<ChunkdictChunkInfo>> = BTreeMap::new();
            ...
            while radius <= max_radius {
                ...
                for chunk in test.iter() {
                    if !all_chunks.contains(chunk) {
                        total_test_set_size += chunk.chunk_compressed_size;
                    }
                }
    ...
    pub fn deduplicate_version(all_chunks: &[ChunkdictChunkInfo]) -> anyhow::Result<(Versiondic, Imagedic)> {
        let train_percentage = 0.7;
        let datadict = Self::deduplicate_image(all_chunks.to_owned())?;
        let (train, test) = Self::devide_set(all_chunks, train_percentage)?;
        let mut a = Vec::new();
        for i in &train {
            if a.contains(&i.version_name.clone()) {
                continue;
            }
        ...
        while threshold <= max_threshold {
            ...
            let mut test_by_image = Self::devide_by_image(&test)?;
            let mut min_test_length = 0;
            for point in test_by_image.iter_mut() {
                if version_datadict.contains_key(&point.image_name.clone()) {
                    ...
                    min_test_length += point.chunk_list.len();
                }
                for chunk in point.chunk_list.iter() {
            ...
            threshold += 0.05;
            info!(
                "test set size is {} and {} and {} and {}",
                min_test_size,
                (min_test_length as f64) / (test.len() as f64),
                min_test_length,
                test.len()
            );
        }
...
impl Ord for CustomString {
    ...
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        let num1: Vec<i32> = self
            .0
            .split('.')
            .filter_map(|segment| segment.parse::<i32>().ok())
            .collect();
        let num2: Vec<i32> = other
            .0
            .split('.')
            .filter_map(|segment| segment.parse::<i32>().ok())
            .collect();
...
impl Table<ChunkdictChunkInfo, DatabaseError> for ChunkTable {
    ...
    fn list_paged(&self, offset: i64, limit: i64) -> Result<Vec<ChunkdictChunkInfo>, DatabaseError> {
        let conn_guard = self
            .conn
            .lock()
            .map_err(|e| DatabaseError::PoisonError(e.to_string()))?;
...
mod tests {
    ...
    #[test]
    fn test_devide_by_image() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let datadict = Algorithm::<SqliteDatabase>::devide_by_image(&all_chunks)?;
    ...
    fn test_devide_set() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (train, test) = Algorithm::<SqliteDatabase>::devide_set(&all_chunks, 0.7)?;
    ...
    fn test_dbscan() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(all_chunks.len(), 20000);
        let mut data_point = Algorithm::<SqliteDatabase>::devide_by_image(&all_chunks)?;
